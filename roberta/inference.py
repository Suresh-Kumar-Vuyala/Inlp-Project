"""
RoBERTa Semantic Expert — Inference & Results Script
TECPE Project (Member 1)

Usage (Google Drive — default):
    python inference.py

Usage (custom paths):
    python inference.py \
        --model_dir /content/drive/MyDrive/NLP_Project/models/roberta_semantic/final_model \
        --processed_dir /content/drive/MyDrive/NLP_Project/processed_data \
        --output_dir /content/drive/MyDrive/NLP_Project/models/roberta_semantic

Outputs (in --output_dir):
    predictions_logits_validation.csv   ← primary deliverable for Member 3
...
# ── Google Drive mount (auto-detected when running in Colab) ──────────────
DRIVE_ROOT    = "/content/drive/MyDrive/NLP_Project"
LOCAL_ROOT    = "./NLP_Project"
DRIVE_MODEL   = f"{DRIVE_ROOT}/models/roberta_semantic/final_model"
LOCAL_MODEL   = f"{LOCAL_ROOT}/models/roberta_semantic/final_model"
DRIVE_PROC    = f"{DRIVE_ROOT}/processed_data"
LOCAL_PROC    = f"{LOCAL_ROOT}/processed_data"
DRIVE_OUTPUT  = f"{DRIVE_ROOT}/models/roberta_semantic"
LOCAL_OUTPUT  = f"{LOCAL_ROOT}/models/roberta_semantic"

def mount_google_drive():
    """Mount Google Drive if running in Colab and not already mounted."""
    try:
        import google.colab  # only available inside Colab
        ...
        # Not in Colab — assume paths are local or already accessible
        pass
...
# ── Label registry (must be identical to train.py) ────────────────────────
EMOTION_LIST = ["anger", "disgust", "fear", "joy", "sadness", "surprise", "neutral"]
NUM_LABELS   = 7

EMOTION_LABELS   = {e: i for i, e in enumerate(EMOTION_LIST)}
LABEL_TO_EMOTION = {i: e for i, e in enumerate(EMOTION_LIST)}
...
class RoBERTaInference:
    ...
    def run(self, dataset, output_dir: str, split: str = "validation"):
        ...
        # Use Trainer just for its predict() — no training args needed
        ...
        csv_path = output_dir / f"predictions_logits_{split}.csv"
        ...
        json_path = output_dir / f"eval_results_{split}.json"
...
# ── Main ──────────────────────────────────────────────────────────────────
def parse_args():
    p = argparse.ArgumentParser(description="RoBERTa inference + results")
...
def main():
    # Mount Drive before parsing args so Drive paths exist when validated
    mount_google_drive()

    args = parse_args()

    print("=" * 60)
    print("ROBERTA SEMANTIC EXPERT — INFERENCE & RESULTS")
    print("=" * 60)
    ...
    # Validate paths exist before doing any heavy work
    if not Path(args.model_dir).exists():
        sys.exit(f"❌ model_dir not found: {args.model_dir}\n"
                 f"   Check your Drive path or pass --model_dir explicitly.")
    dataset_path = Path(args.processed_dir) / "hf_dataset"
    if not dataset_path.exists():
        sys.exit(f"❌ HF dataset not found at: {dataset_path}\n"
                 f"   Run train.py first, or pass --processed_dir explicitly.")
    ...
    # Inference
    inferer  = RoBERTaInference(args.model_dir, max_length=args.max_length)
    tokenized = inferer.prepare_dataset(split_dataset)
    df, results = inferer.run(tokenized, args.output_dir, split=args.split)

    print("\n✅ INFERENCE COMPLETE")
    print(f"   Primary deliverable → {args.output_dir}/predictions_logits_{args.split}.csv")
